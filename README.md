...
- apt-get 명령
 Docker는 각 RUN 명령을 실행할 때마다 새로운 레이어를 생성합니다. apt-get 명령을 한 줄로 작성하면 레이어 수를 줄일 수 있으며, 이전에 실행한 명령과 동일한 경우 캐시된 이미지를 재사용하므로 이미지 빌드 속도가 빨라질 수 있습니다. 그러나 한 줄로 작성하면 디버깅이 어려워질 수 있습니다. 따라서 상황에 맞게 결정하는 것이 중요하다는 것을 깨달았습니다.
  ```
    RUN apt-get update && \
